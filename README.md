# Ο πρώτος Διαμεταγλωτιστής της ΓΛΩΣΣΑΣ

Ο *glwssa-compiler* μεταγλωτίζει οποιοδήποτε πρόγραμμα υπάρχει στην ΓΛΩΣΣΑ σε αρχείο `*.exe`/`*.out` με την βοήθεια της C++ και της Python.

Ο τρόπος λειτουργίας του είναι ο εξής:
- Διαβάζει τον κώδικα με την βοήθεια της Python
- Δημιουργεί ένα νέο αρχείο μεταφράζοντας όλο το πρόγραμμα της ΓΛΩΣΣΑΣ αλλά με C++ σύνταξη.
- Τέλος, μεταγλωττίζει την C++ με το αποτέλεσμα να είναι ένα αρχείο `*.exe`/`*.out`.

## Γιατί αυτές οι Γλώσσες προγραμματισμού;
- ΓΛΩΣΣΑ:
    Τυγχαίνει να ήμουν στην Γ' Λυκείου, και δεν έβρισκα πουθενά κάποιο πρόγραμμα που να μεταγλωτίζει σε `*.exe`/`*.out` προγράμματα στην γλώσσα, σαν τις υπόλοιπες γλώσσες προγραμματισμού.

- Python:
    Είναι η μοναδική γλώσσα που έχω την μεγαλύτερη εμπειρία (4 χρόνια). Και είναι 1000 φορές πιο εύκολο να γράφεις την λογική του μεταφραστή με αυτήν.

- C++:
    Είναι η πρώτη μου γλώσσα προγραμματισμού που έχω μάθει. Είναι γρήγορη και έχει κάποιες λειτουργίες που η Python δεν έχει. Επίσης, γιατί να μην κάνουμε το πρόγραμμα αυτό της ΓΛΩΣΣΑΣ το πιο γρήγορo που υπάρχει ;)

## Έκδοσεις
Τις εκδόσεις του *glwssa-compiler* μπορείς να τις βρεις [εδώ](https://github.com/theolaos/glwssa-compiler/releases) (δυστηχώς προς το παρόν δεν υπάρχει κάποια έκδοση διαθέσιμη). Δεν χρειάζεσαι κάποιο εξτρά πρόγραμμα ή βιβλιοθήκη για να τρέξεις τις εκδόσεις του *glwssa-compiler*.

Αμα θέλεις να τρέξεις τον Compiler από τον πηγαίο κώδικα, τότε επιβεβαίωσε πως έχεις εγκατεστημένη την πιο πρόσφατη έκδοση της Python, ομoίως και για το gcc (μεταγλωτιστής της C++ - καθώς και άλλων γλωσσών - από την ΕΛ/ΛΑΚ ομάδα GNU)

## TODO

Το παλιό parser χωρίς AST, μεταφράζει κι όλας:
- Από την θεωρία της Γ' Λυκείου
    - [x] : Υποστήριξει βασικών συναρτήσεων ΓΛΩΣΣΑΣ (Α_Τ(), Α_Μ(), Τ_Ρ())
        - [x] : Υλοποίηση Α_Τ(), Α_Μ(), Τ_Ρ()
    - [x] : Δομή Ακολουθίας
    - [x] : Δομή Επιλογής
        - [x] : Απλή Επιλογή
        - [x] : Συνθετη Επιλογή
        - [x] : Πολλαπλή Επιλογή

Το καινούργιο parser με AST, ΑΝΑΓΝΩΡΙΖΕΙ τα παρακάτω:
- Από την θεωρία της Γ' Λυκείου, 
    - [ ] : Υποστήριξει βασικών συναρτήσεων ΓΛΩΣΣΑΣ (Α_Τ(), Α_Μ(), Τ_Ρ() κλπ)
        - [ ] : Υλοποίηση Α_Τ(), Α_Μ(), Τ_Ρ(), ΗΜ(), ΣΥΝ(), ΕΦ()
    - [X] : Δομή Ακολουθίας
        - [X] : Δημιουργία μεταβλητών
        - [X] : Εκτύπωση
        - [X] : Εισαγωγή δεδομένων
        - [X] : Εκχώρηση Δεδομένων
    - [ ] : Δομή Επιλογής
        - [X] : Απλή Επιλογή
        - [X] : Συνθετη Επιλογή
        - [X] : Πολλαπλή Επιλογή
        - [X] : Εμφωλευμένη
        - [ ] : Επίλεξε
    - [X] : Δομή Επανάληψης
        - [X] : ΟΣΟ
        - [X] : ΓΙΑ
        - [X] : ΜΕΧΡΙΣ_ΟΤΟΥ
    - [ ] : Πίνακες
    - [ ] : Τμηματικός Προγραμματισμός
        - [ ] : ΥΠΟΠΡΟΓΡΑΜΜΑΤΑ
        - [ ] : ΣΥΝΑΡΤΗΣΗ


- [ ] : QoL
    - [x] : Υποστήριξει ελληνικών και αγγλικών χαρακτήρων για την δημιουργία και ονομασία Υποπρογραμμάτων και μεταβλητών.
    - [ ] : Κάλυψη όλων των μηνύματων για σφάλματα
    - [ ] : Κάποιο optimization για το parsing expressions. (ένα άμεσο είναι να μην δημιουργεί τόσες πολλές μεταβλητές ανα κάθε μέθοδος)
    - [ ] : Οταν κάνει parse τους λογικούς τελεστές, καλό θα ήταν όταν βγαίνει κάποιο σφάλμα μεταβλητών να λέει μήπως εννοούσες "Και"? Το έγραψες στα αγγλικά.
    - [ ] : Να βρω όλα τα πιθανά edge cases με τον tokenizer.
    - [ ] : Explicit Behaviour.-

- Compile προγράμματα από άλλες εφαρμογές
    - [ ] : Διερμηνευτής γλώσσας
    - [ ] : Γλωσσομάθεια


## Θες να βοηθήσεις;
Αν γνωρίζεις C++ και Python, και θες να βοηθίσεις στην δημιουργία ενός, σχεδόν, αληθινού μεταγλωτιστή για την γλώσσα προγραμματισμου που διδάσκεται στα σχολεία, είσαι ευπρόσδεκτος. Διάβασε παραπάνω για το τι δεν έχει έχει υλοποιηθεί ακόμα στον μεταγλωτιστή, κάνε τις αλλαγές σου και ένα pull request.


## Παραγόμενος Κώδικας

Ο κώδικας που παράγεται από αυτόν τον διαμεταγλωττιστή (δηλαδή από το πρόγραμμα *glwssa-compiler*) **δεν καλύπτεται** από την άδεια GNU GPL. Οι χρήστες διατηρούν πλήρη δικαιώματα πάνω στον παραγόμενο κώδικα, συμπεριλαμβανομένων χρήσεων για ακαδημαϊκούς, προσωπικούς ή εμπορικούς σκοπούς.


## Άδεια λογισμικού

Η άδεια του παρόντος λογισμικού είναι η Γενική Άδεια Δημόσιας Χρήσης GNU (GNU General Public License), συγκεκριμένα η έκδοση GNU GPLv3, και περιλαμβάνεται στο αρχείο `LICENSE`.

Το αρχείο `LICENSE` αποτελεί τη δεσμευτική άδεια που εφαρμόζεται σε ολόκληρο το έργο *glwssa-compiler*.

Αν δεν γνωρίζεις αγγλικά, μπορείς να διαβάσεις τη μεταφρασμένη (μη επίσημη) έκδοση της άδειας εδώ:
https://mathe.ellak.gr/gpl-3-0-txt/