...
class ParseError(Exception):
    """ParseError: Parsing error. While parsing, did not expect that error.
...
class ParserAST:
    ...
    def current_token(self, index: int=0, default: Token = Token("NO_TOKEN","NO_VALUE", -1, -1, -1, -1)) -> Token:
        ...
        if self.current_line >= len(self.program_tokens):
            ...
            self.current_line += 1
            return Token("EOF","EOF", -1, line, -1, -1)
    ...
    # def fnext_line(self, msg: str) -> None:
    ...
    def parse_variables_block(self, branch: _Union[Block, Program]) -> None:
        ...
        self.check_eof("Expected to find token START")
